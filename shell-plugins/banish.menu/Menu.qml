...
Item {
  ...
  onAppLibraryChanged: if (appLibrary && root.providersLoaded["apps"]) root.mergeAppRows()

  // Some hosts leave cloned menus without the shared application API.
  // Load the same installed implementation only when that API is absent.
  Loader {
  ...
  // Compact spacing follows dos-moos/walker.css; other themes keep stock sizing.
  readonly property bool compact: Color.shellValues["menu.compact"] === "true"
  readonly property int cornerRadius: Number(Color.pick("menu.border-radius", String(Style.cornerRadius)))
  readonly property int rowRadius: cornerRadius
  readonly property int iconSize: compact ? Style.space(18) : Style.font.iconLarge
  readonly property int iconColumnWidth: compact ? iconSize : Style.space(36)
  property int contentMargin: compact ? Style.space(10) : Style.spacing.panelPadding
  readonly property int extraBottomPadding: Style.space(Number(Color.pick("menu.extra-bottom-padding", "0")))
  property int headerHeight: compact ? Math.max(Style.space(33), Style.font.heading + Style.space(17)) : Math.max(Style.space(34), Style.font.title + Style.spacing.controlPaddingY * 2)
  property int contentSpacing: compact ? Style.space(14) : Style.spacing.md
  property int baseRowHeight: compact ? Math.max(Style.space(36), Style.font.heading + Style.space(20)) : Math.max(Style.space(50), Style.font.body + Style.spacing.rowPaddingX * 2)
  property int detailRowHeight: compact ? baseRowHeight + Style.font.bodySmall + Style.space(5) : Math.max(Style.space(58), Style.font.body + Style.font.caption + Style.spacing.rowPaddingX * 2)
  ...
  property int rowSpacing: compact ? 0 : Style.spacing.xs
  property int dividerHeight: Style.space(17)
  property bool searchDivider: false
  property int layoutSerial: 0
  // Walker's menu command supplied the outer width; its CSS minimum did not.
  readonly property int menuWidth: Style.space(Number(Color.pick("menu.width", compact ? "295" : "300")))
  readonly property real verticalBorderInset: compact ? Border.top(borderSpec) + Border.bottom(borderSpec) : 0
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : ((root.activeMenu === "trigger.capture.screenrecord" || root.activeMenu === "style.font") ? Style.space(520) : menuWidth), panel.width - Style.gapsOut * 2)
  property int visibleRowsHeight: root.dmenuActive ? dmenuRowListHeight(layoutSerial, displayModel.count, filterText) : rowListHeight(layoutSerial, displayModel.count, filterText, searchDivider)
  property int cardHeight: root.dmenuActive
    ? Math.min(contentMargin * 2 + extraBottomPadding + verticalBorderInset + headerHeight + (mode === "input" ? 0 : contentSpacing + visibleRowsHeight), panel.height - Style.gapsOut * 2)
    : Math.min(contentMargin * 2 + extraBottomPadding + verticalBorderInset + headerHeight + contentSpacing + visibleRowsHeight, panel.height - Style.gapsOut * 2)
  ...
  function availableRowsHeight() {
    ...
    var available = panel.height - top - Style.gapsOut - root.contentMargin * 2 - root.extraBottomPadding - root.verticalBorderInset - root.headerHeight - root.contentSpacing
  ...
  PanelWindow {
    ...
    RectangularShadow {
      anchors.fill: card
      visible: Color.shellValues["menu.floating-shadow"] === "true"
      radius: root.cornerRadius
      blur: Number(Color.pick("floating-shadow.blur", "18"))
      spread: Number(Color.pick("menu.shadow-spread", "2"))
      offset: Qt.vector2d(0, Number(Color.pick("floating-shadow.offset-y", "2")))
      color: Qt.rgba(0, 0, 0, Color.pickAlpha("menu.shadow-opacity", Color.pickAlpha("floating-shadow.opacity", 0.55)))
      cached: true
    }
    ...
    BorderSurface {
      ...
      bottomPadding: root.contentMargin + root.extraBottomPadding
      ...
      Column {
        ...
        Rectangle {
          ...
          Text {
            ...
            anchors.leftMargin: root.compact ? Style.space(8) : 0
            anchors.rightMargin: root.compact ? Style.space(8) : 0
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              Behavior on color { enabled: root.compact; ColorAnimation { duration: 150 } }
              ...
              Column {
                ...
                anchors.leftMargin: row.hasIcon ? Style.space(root.compact ? 10 : 6) : root.rowReservedBorderLeft + Style.space(root.compact ? 8 : 18)
